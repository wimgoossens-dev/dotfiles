...
(defun dotspacemacs/user-config ()
  ...
  (let ((colemak-keys
         '(("m" . evil-backward-char)
           ...
           ("e" . evil-next-line)))
        (movement-states '(normal motion visual operator))
        (command-states '(normal motion visual)))

    ;; Remove hjkl
    (dolist (state movement-states)
      (dolist (key '("h" "j" "k" "l"))
        (evil-global-set-key state (kbd key) nil)))

    ;; Install Colemak movement
    (dolist (state movement-states)
      (dolist (binding colemak-keys)
        (evil-global-set-key
         state
         (kbd (car binding))
         (cdr binding))))
  ...
  (dolist (state '(normal motion visual))
    (evil-global-set-key state (kbd "l") #'evil-insert)
    (evil-global-set-key state (kbd "k") #'evil-ex-search-next)
    (evil-global-set-key state (kbd "K") #'evil-ex-search-previous)
    (evil-global-set-key state (kbd "j") #'evil-forward-word-end)
    (evil-global-set-key state (kbd "h") #'evil-set-marker))

  ;; Insert escape sequence
  (with-eval-after-load 'evil-escape
    (setq evil-escape-key-sequence "wq"
          evil-escape-delay 0.29)
    (add-to-list 'evil-escape-inhibit-functions (lambda () (minibufferp))))

  ) ;; This single parenthesis MUST be the absolute last thing in your user-config!
